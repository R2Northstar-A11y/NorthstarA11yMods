...
void function InitServerBrowserMenu()
{
	...
	AddButtonEventHandler( Hud_GetChild( file.menu, "BtnServerJoin"), UIE_GET_FOCUS, FocusHandler_Join )
	...
	AddButtonEventHandler( Hud_GetChild( file.menu, "BtnFiltersClear"), UIE_GET_FOCUS, FocusHandler_Clear )
	...
	AddButtonEventHandler( Hud_GetChild( file.menu, "BtnServerNameTab"), UIE_GET_FOCUS, FocusHandler_ServerNameTab )
	AddButtonEventHandler( Hud_GetChild( file.menu, "BtnServerPlayersTab"), UIE_GET_FOCUS, FocusHandler_ServerPlayersTab )
	AddButtonEventHandler( Hud_GetChild( file.menu, "BtnServerMapTab"), UIE_GET_FOCUS, FocusHandler_ServerMapTab )
	AddButtonEventHandler( Hud_GetChild( file.menu, "BtnServerGamemodeTab"), UIE_GET_FOCUS, FocusHandler_ServerGamemodeTab )
	AddButtonEventHandler( Hud_GetChild( file.menu, "BtnServerLatencyTab"), UIE_GET_FOCUS, FocusHandler_ServerLatencyTab )
	...
	AddButtonEventHandler( Hud_GetChild( file.menu, "SwtBtnSelectMap"), UIE_GET_FOCUS, FocusHandler_SelectMap )
	AddButtonEventHandler( Hud_GetChild( file.menu, "SwtBtnSelectGamemode"), UIE_GET_FOCUS, FocusHandler_SelectGamemode )
	AddButtonEventHandler( Hud_GetChild( file.menu, "SwtBtnHideFull"), UIE_GET_FOCUS, FocusHandler_HideFull )
	AddButtonEventHandler( Hud_GetChild( file.menu, "SwtBtnHideEmpty"), UIE_GET_FOCUS, FocusHandler_HideEmpty )
	AddButtonEventHandler( Hud_GetChild( file.menu, "SwtBtnHideProtected"), UIE_GET_FOCUS, FocusHandler_HideProtected )
	AddButtonEventHandler( Hud_GetChild( file.menu, "BtnSearchLabel"), UIE_GET_FOCUS, FocusHandler_Search )
	AddButtonEventHandler( Hud_GetChild( file.menu, "BtnServerSearch"), UIE_GET_FOCUS, FocusHandler_Search )

	AddButtonEventHandler( Hud_GetChild( file.menu, "BtnServerSearch"), UIE_CHANGE, FilterAndUpdateList )

	AddButtonEventHandler( Hud_GetChild( file.menu, "BtnServerSearch"), UIE_CHANGE, ChangeHandler_SearchType )
...
void function SayServerInfo(int scriptID) {
	...
	if(server.serverProtected)
		...
		thread TTSsay(NSGetServerName( server.serverIndex )+", " + server.serverPlayers + " of " + server.serverPlayersMax + ", " + Localize(server.serverGamemode) + " on " + Localize("#"+server.serverMap))
}
...
void function FocusHandler_Join( var button ) {
	thread TTSsay(Localize("#JOIN_BUTTON"))
}
void function FocusHandler_Clear( var button ) {
	thread TTSsay(Localize("#CLEAR_FILTERS"))
}
void function FocusHandler_ServerNameTab( var button ) {
	thread TTSsay(Localize("#SERVERS_COLUMN"))
}
void function FocusHandler_ServerPlayersTab( var button ) {
	thread TTSsay(Localize("#PLAYERS_COLUMN"))
}
void function FocusHandler_ServerMapTab( var button ) {
	thread TTSsay(Localize("#MAP_COLUMN"))
}
void function FocusHandler_ServerGamemodeTab( var button ) {
	thread TTSsay(Localize("#GAMEMODE_COLUMN"))
}
void function FocusHandler_ServerLatencyTab( var button ) {
	thread TTSsay(Localize("#LATENCY_COLUMN"))
}
void function FocusHandler_SelectMap( var button ) {
	thread TTSsay(Localize("#MAP_FILTER") + ", " + Localize("#"+filterArguments.filterMaps[ GetConVarInt( "filter_map" ) ]))
}
void function FocusHandler_SelectGamemode( var button ) {
	thread TTSsay(Localize("#GAMEMODE_FILTER") + ", "+ Localize("#"+filterArguments.filterGamemodes[ GetConVarInt( "filter_gamemode" ) ]))
}
void function FocusHandler_HideFull( var button ) {
	thread TTSsay(Localize("#HIDE_FULL_FILTER") + ", " + Localize(GetConVarBool( "filter_hide_full" ) ? "#SWITCH_YES" : "#SWITCH_NO"))
}
void function FocusHandler_HideEmpty( var button ) {
	thread TTSsay(Localize("#HIDE_EMPTY_FILTER") + ", " + Localize(GetConVarBool( "filter_hide_empty" ) ? "#SWITCH_YES" : "#SWITCH_NO"))
}
void function FocusHandler_HideProtected( var button ) {
	thread TTSsay(Localize("#HIDE_PROT_FILTER") + ", " + Localize(GetConVarBool( "filter_hide_protected" ) ? "#SWITCH_YES" : "#SWITCH_NO"))
}
void function FocusHandler_Search( var button ) {
	thread TTSsay(Localize("#SEARCHBAR_LABEL"))
}
int searchLength = 0
void function ChangeHandler_SearchType( var button ) {
	string searchTerm = Hud_GetUTF8Text( Hud_GetChild( file.menu, "BtnServerSearch" ) )
	if(searchTerm.len() < searchLength) {
		thread TTSsay("Back")
	} else {
		thread TTSsay(searchTerm.slice(-1).toupper())
	}
	searchLength = searchTerm.len()
}